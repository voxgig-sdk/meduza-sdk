...
class MeduzaMakeError
{
    public static function call(?MeduzaContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MeduzaContext([], null);
        }
        $op = $ctx->op ?? new MeduzaOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MeduzaResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MeduzaError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MeduzaError('', $msg, $ctx);
