...
class FtCtrl extends MeduzaControl
{
...
class FtClient
{
    ...
    private MeduzaUtility $utility;

    public function __construct(MeduzaUtility $utility, array $options)
    {
    ...
    public function get_utility(): MeduzaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MeduzaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MeduzaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MeduzaConfig::make_feature($name);
    ...
    private static function buildUrl(MeduzaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MeduzaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MeduzaResponse($fetched);
        }
    ...
    private function fail(MeduzaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MeduzaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
