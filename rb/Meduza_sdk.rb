...
class MeduzaSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MeduzaHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MeduzaHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MeduzaFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MeduzaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MeduzaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MeduzaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    allow_op = VoxgigStruct.getpath(@options, "allow.op")
    {
      ...
      "err" => MeduzaError.new(
        "#{op}_allow",
        "MeduzaSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = MeduzaHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MeduzaError.new(
        "graphql_error", "MeduzaSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
