...
class NewEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MEDUZA_TEST_NEW_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def new_basic_setup(extra)
  ...
  entid_env_raw = ENV["MEDUZA_TEST_NEW_ENTID"]
  ...
  if env["MEDUZA_TEST_LIVE"] == "TRUE"
    ...
    client = MeduzaSDK.new(Helpers.to_map(merged_opts))
  end
