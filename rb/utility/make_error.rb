...
module MeduzaUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = MeduzaContext.new({}, nil)
    end
    op = ctx.op || MeduzaOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || MeduzaResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(MeduzaError) ? err.msg : err.to_s
    ...
    sdk_err = MeduzaError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(MeduzaError)
