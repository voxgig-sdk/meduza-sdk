...
class NewEntity
  ...
  def data_set(args)
    if args
      @_data = MeduzaHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = MeduzaHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<New>, Array] the matching New items; raises MeduzaError on failure
