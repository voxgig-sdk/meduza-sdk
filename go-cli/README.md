...
boru-driven command-line client **and** interactive REPL for the Meduza
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/meduza-cli)
make build
...
MEDUZA_BASE=https://api.example.com ./meduza-cli list new
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/meduza-cli
   ```
...
Run with no arguments to open a REPL (prompt `meduza>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/meduza-cli`. |
...
(`./meduza-cli <expr>`) and interactively (the REPL), and expressions compose the
