// Command Meduza-mcp is an MCP server exposing the Meduza
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Meduza_list  — list records of an entity
//	Meduza_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: MEDUZA_APIKEY carries the API key and
	// MEDUZA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MEDUZA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MEDUZA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMeduzaSDK(opts)
