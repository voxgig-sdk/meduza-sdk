...
[MCP](https://modelcontextprotocol.io) server exposing the Meduza SDK as
two agent tools — `meduza_list` and `meduza_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/meduza-mcp)
make build

# 2. Provide credentials via the environment
export MEDUZA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user meduza \
  -- /absolute/path/to/meduza-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./meduza-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// meduza_list: first page of records
{ "entity": "new" }
{ "entity": "new", "query": { } }

// meduza_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/meduza-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user meduza \
     -- "$PWD"/dist/*/meduza-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `meduza_list` and `meduza_load` tools now appear
   in new sessions. Ask the agent to *"list new using meduza"*
   and it calls `meduza_list` with `{"entity":"new"}`.
...
```sh
export MEDUZA_APIKEY=sk_live_xxx            # API key
export MEDUZA_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `meduza_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `meduza_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MEDUZA_APIKEY` | API key sent with every request. |
| `MEDUZA_BASE` | Optional override of the API base URL. |
...
```sh
./meduza-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"meduza_load","arguments":{"entity":"new","query":{"id":1}}}}'
```
