...
import (
	...
	omni "github.com/voxgig-sdk/meduza-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MeduzaError); is {
				out := map[string]any{
					"name":    "MeduzaError",
...
func omniCtx(arg any, client *sdk.MeduzaSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MeduzaSDK); is {
			client = live
