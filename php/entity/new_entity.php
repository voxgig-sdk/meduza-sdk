...
class NewEntity
{
    ...
    {
        if ($args) {
            $this->_data = MeduzaHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = MeduzaHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws MeduzaError on failure (item-5 convention).
     */
