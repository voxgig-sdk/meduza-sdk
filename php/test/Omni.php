...
//    only - a typed MeduzaContext there would make every ctx
...
final class MeduzaOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new MeduzaContext($clean, null);
    ...
    public static function err_from_map($m): ?MeduzaError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MeduzaError(is_string($code) ? $code : '', $msg);
    }
