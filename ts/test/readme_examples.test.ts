...
function isRunnable(code: string): boolean {
  return (
    /new\s+MeduzaSDK\b/.test(code) ||
    /\bMeduzaSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MeduzaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MeduzaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
